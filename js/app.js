...
angular.module('app').config(function ($stateProvider) {
  ...
  var templateUrl = function templateUrl(url) {
    return "".concat(url, "?ts=1624507032446");
  };
...
angular.module('app').controller('CheckedInController', function ($state, $localStorage) {
  ...
  var month = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'][ts.getMonth()];
  var year = ts.getFullYear();
  var minutes = ts.getMinutes().toString().padStart(2, 0);
  var hours;

  if (ts.getHours() > 12) {
    hours = (ts.getHours() - 12).toString().padStart(2, 0);
    vm.timestamp = "".concat(day, " ").concat(month, " ").concat(year, " ").concat(hours, ":").concat(minutes, "pm");
  } else {
    hours = ts.getHours().toString().padStart(2, 0);
    vm.timestamp = "".concat(day, " ").concat(month, " ").concat(year, " ").concat(hours, ":").concat(minutes, "am");
  }
